...
namespace Presenter.Controllers
{
    ...
    public class SellersController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<Envelope> CreateSeller([FromBody] CreateSellerRequest request, CancellationToken cancellationToken)
        {
            var command = new CreateSellerCommand(request.ClientId);
            var result = await _createSellerHandler.HandleAsync(command, cancellationToken);
        ...
        [HttpGet("{id}")]
        public async Task<Envelope> GetSeller(Guid id, CancellationToken cancellationToken)
        {
            var query = new GetSellerByIdQuery(id);
            var result = await _getSellerByIdHandler.HandleAsync(query, cancellationToken);
            if (result.IsFailure)
            {
        ...
        [HttpGet]
        public async Task<Envelope> GetSellers([FromQuery] SearchSellersQuery query, CancellationToken cancellationToken)
        {
            var result = await _searchSellersHandler.HandleAsync(query, cancellationToken);
            if (result.IsFailure)
            {
                return new Envelope(HttpStatusCode.BadRequest, error: result.Error);
            }
        ...
        [HttpPut("{id}")]
        public async Task<Envelope> UpdateSeller(Guid id, [FromBody] UpdateSellerRequest request, CancellationToken cancellationToken)
        {
            var command = new UpdateSellerCommand(id, request.ClientId);
            var result = await _updateSellerHandler.HandleAsync(command, cancellationToken);
        ...
        [HttpDelete("{id}")]
        public async Task<Envelope> DeleteSeller(Guid id, CancellationToken cancellationToken)
        {
            var command = new DeleteSellerCommand(id);
            var result = await _deleteSellerHandler.HandleAsync(command, cancellationToken);
